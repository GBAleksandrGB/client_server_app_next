...
def main():
    ...
    while True:
        ...
        if action == 'q':
            ...
            process.append(subprocess.Popen(f'{PYTHON_PATH} {BASE_PATH}/main_server.py',
                                            creationflags=subprocess.CREATE_NEW_CONSOLE))
        elif action == 'k':
            clients_count = int(input('Введите количество тестовых клиентов для запуска: '))
            for i in range(clients_count):
                process.append(subprocess.Popen(f'{PYTHON_PATH} {BASE_PATH}/main_client.py -n test{i + 1} -p 123456',
                                                creationflags=subprocess.CREATE_NEW_CONSOLE))
        elif action == 'x':
            while process:
                process.pop().kill()
